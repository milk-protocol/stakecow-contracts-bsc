...
contract GrowingCow is StakePool {
    ...
    ) public initializer 
    {
        ...
        finishTime = _starttime.add(_totalPeriods.mul(_duration));
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkhalve checkStart{
        ...
        require(block.timestamp < finishTime, "ERROR: Stake over");
        super._stake(amount);
    ...
    modifier checkhalve(){
        if (block.timestamp >= periodFinish && block.timestamp < finishTime) {
            initreward = initreward.add(initreward.div(10));
